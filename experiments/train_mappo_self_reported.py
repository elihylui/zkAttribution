"""MAPPO training on SocialJax Cleanup with self-reported attribution.

Copy of `train_mappo_oracle_attribution.py` with the Stage 4 changes:
  1. `SelfReportWrapper` instead of `AttributionWrapper` — each agent broadcasts
     its own *claimed* alpha; peers see the claims unverified. The true alpha is
     logged (claimed / true / inflation) but never shown to agents.
  2. The `Actor` gains a second head — a Categorical over claim buckets — so the
     claim is a learned action. The PPO loss sums both heads' log-probs over the
     factored (env-action, claim) action space.

Implements Stage 4 of the brief — the "self-reported baseline" regime.
"""

import os
import sys

# Make zkattribution + socialjax importable when this script is run from the repo root.
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
sys.path.insert(0, os.path.join(_REPO_ROOT, "src"))
sys.path.insert(0, os.path.join(_REPO_ROOT, "external", "SocialJax"))
...
from zkattribution.wrappers import SelfReportWrapper

# Discrete buckets the self-report claim head emits. Claimed alpha is
# bucket / (NUM_CLAIM_BUCKETS - 1), so 11 buckets -> {0.0, 0.1, ..., 1.0}.
NUM_CLAIM_BUCKETS = 11
import copy
import pickle
import os
...
class Actor(nn.Module):
    action_dim: int
    num_claim_buckets: int = NUM_CLAIM_BUCKETS
    activation: str = "relu"
...
def make_train(config):
    ...
    # Self-reported attribution: agents broadcast their own (unverified) claimed alpha.
    env = SelfReportWrapper(
        env,
        cleanup_events_batch,
        window_size=int(config.get("ALPHA_WINDOW_SIZE", 100)),
        num_claim_buckets=NUM_CLAIM_BUCKETS,
    )
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                pi, pi_claim = actor_network.apply(train_states[0].params, obs_batch)
                _rng, _rng_claim = jax.random.split(_rng)
                action = pi.sample(seed=_rng)
                claim = pi_claim.sample(seed=_rng_claim)
                # Joint log-prob over the factored (env-action, claim) action space.
                log_prob = pi.log_prob(action) + pi_claim.log_prob(claim)
                env_act = [
                    v for v in unbatchify(
                        action, env.agents, config["NUM_ENVS"], env.num_agents
                    ).values()
                ]
                claim_act = [
                    v for v in unbatchify(
                        claim, env.agents, config["NUM_ENVS"], env.num_agents
                    ).values()
                ]
                ...
                )(rng_step, env_state, (env_act, claim_act))
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _actor_loss_fn(actor_params, traj_batch, gae):
                        # RERUN NETWORK
                        pi, pi_claim = actor_network.apply(
                            actor_params,
                            jnp.reshape(traj_batch.obs, (-1, *(traj_batch.obs).shape[-3:])),
                        )

                        # Joint log-prob over the factored (env-action, claim) space.
                        log_prob = pi.log_prob(
                            traj_batch.action.reshape(-1)
                        ) + pi_claim.log_prob(traj_batch.claim.reshape(-1))
                        ...
                        entropy = (pi.entropy() + pi_claim.entropy()).mean()
                        # debug
...
def single_run(config):
    ...
    # Regime-prefixed so each regime's checkpoint is distinct.
    filename = f'self_reported_{config["ENV_NAME"]}_seed{config["SEED"]}_reward_{config["REWARD"]}'
...
def evaluate(params, env, save_path, config):
    ...
    pics.append(img)
    root_dir = f"evaluation/self_reported_cleanup"
    ...
    for o_t in range(config["GIF_NUM_FRAMES"]):
        ...
        # 使用模型选择动作
        network = Actor(action_dim=env.action_space().n, activation=config["ACTIVATION"])
        pi, pi_claim = network.apply(params, obs_batch)
        rng, _rng = jax.random.split(rng)
        actions = pi.sample(seed=_rng)
        rng, _rng = jax.random.split(rng)
        claims = pi_claim.sample(seed=_rng)

        # 转换动作格式
        env_act = {k: v.squeeze() for k, v in unbatchify(
            actions, env.agents, 1, env.num_agents
        ).items()}
        claim_act = {k: v.squeeze() for k, v in unbatchify(
            claims, env.agents, 1, env.num_agents
        ).items()}
        ...
        obs, state, reward, done, info = env.step(
            ...
            (
                ...
                [v.item() for v in claim_act.values()],
            ),
        )
        done = done["__all__"]
        ...
        # AttributionState wraps the inner env state; agent_locs lives on env_state.
        inner_state = getattr(state, "env_state", state)
        print(f'State: {inner_state.agent_locs}')
        print("###################")
...
@hydra.main(
    version_base=None,
    config_path="../external/SocialJax/algorithms/MAPPO/config",
    config_name="mappo_cnn_cleanup",
)
def main(config):
    if config["TUNE"]:
        tune(config)
